...
export class Utility {
	...
	public static notify(text: string = "undefined", mode = "i", ...items: string[]) {
		switch (mode) {
			case "i": return vscode.window.showInformationMessage(text, ...items);
			case "w": return vscode.window.showWarningMessage(text, ...items);
			case "e": return vscode.window.showErrorMessage(text, ...items);
		}
	}
}
